...
Communicating with zUID is dead simple: 
  ...
  - Response will be a multi frame message. Each frame will contain one number in ASCCI string
...
Notes
--------------
  - zUID heavily depends on NTP. Least significant 45 bit of a number is timestamp in millisecond. Be sure to run reliable NTP service on your zUID cluster. on Linux you can run `ntpd` or `chronyd`.
